...
test("application shell delegates matching and PWA lifecycle to feature modules", async () => {
  const [app, matching, messages, network, projects, workspace, communities, profile, dialogs, operations, coordination, notifications, clickHandler, pwa, deployment, worker] = await Promise.all([
  ...
  assert.ok(app.split(/\r?\n/).length < 2775, "app.js must keep shrinking as feature modules are extracted");
});
