...
import { acceptOffer, addProjectUpdate, backendConfigured, closeRequest, createRequest as createRemoteRequest, deactivateMyAccount, exportMyData, getAgreementAmendments, getEvidenceUrl, getMyAgreements, getMyProfile, getNotificationPreferences, getNotifications, getProjectMessages, getProjectUpdates, getRequestMedia, getRequestOffers, getSession, handleCompletion, listPublicRequests, markNotificationsRead, performAgreementAction, proposeAmendment, respondAmendment, saveNotificationPreferences, sendProjectMessage, signInWithEmail, signOut, submitOffer, submitReview, updateMyProfile, updateRequest, uploadRequestMedia, uploadWorkEvidence } from "./modules/backend.js";
...
async function loadRemoteWorkspace() {
  ...
  const mapped = requests.map(mapRemoteRequest);await Promise.all(mapped.map(async(request)=>{request.media=await getRequestMedia(request.id)}));
