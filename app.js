...
import { acceptOffer, addProjectUpdate, backendConfigured, closeRequest, createRequest as createRemoteRequest, deactivateMyAccount, exportMyData, getAgreementAmendments, getAgreementHistory, getEvidenceUrl, getMyAgreements, getMyOffers, getMyProfile, getMyRequests, getNotificationPreferences, getNotifications, getProjectMessages, getProjectUpdates, getRequestMedia, getRequestOffers, getSession, handleCompletion, listPublicRequests, manageMilestone, markNotificationsRead, performAgreementAction, proposeAmendment, requestLifecycleAction, respondAmendment, reviseOffer, saveNotificationPreferences, sendProjectMessage, signInWithEmail, signOut, submitOffer, submitReview, updateMyProfile, updateRequest, uploadRequestMedia, uploadWorkEvidence, withdrawOffer } from "./modules/backend.js";
...
function renderDetail(request) {
  ...
  return shell(`<button class="back" data-nav="discover">← Back to requests</button>
    <div class="detail-grid"><article class="detail-main">
      ...
      ${request.media?.length?`<div class="request-media">${request.media.map((item)=>`<figure><img src="${esc(item.url)}" alt="${esc(item.caption||request.title)}"><figcaption>${esc(item.caption||"")}</figcaption></figure>`).join("")}</div>`:""}
...
function agreementControls(agreement, confirmed) {
  ...
  return `${next ? `<button class="secondary full" data-agreement="${next[0]}">${next[1]}</button>` : ""}${completion}${!["completed", "cancelled", "disputed"].includes(agreement.status) ? `<button class="text-btn full" data-action="amend">Propose amendment</button><div class="agreement-links"><button data-agreement="disputed">Raise concern</button><button data-agreement="cancelled">Cancel</button></div>` : ""}`;
}
...
function milestones(request) {
  const planning=["proposed","agreed","scheduled"].includes(request.status);return `<section><span class="eyebrow">Milestones</span><div class="milestones">${request.milestones.map((m, index) => `<button data-milestone="${state.remote ? m.id : index}" class="${m.done || m.completed_at ? "done" : ""}"><span>${m.done || m.completed_at ? "✓" : index + 1}</span>${esc(m.title)}${m.due_at?`<small>${new Date(m.due_at).toLocaleDateString()}</small>`:""}</button>`).join("")}</div>${planning&&state.remote?`<button class="text-btn" data-action="add-milestone">Add milestone</button>`:""}${request.status !== "completed" ? `<button class="text-btn" data-action="hold">Add dependency hold</button>` : `<button class="text-btn" data-action="review">Leave contextual feedback</button>`}</section>`;
}
