...
import { acceptOffer, addProjectUpdate, askProposalQuestion, backendConfigured, closeRequest, createRequest as createRemoteRequest, deactivateMyAccount, exportMyData, getAgreementAmendments, getAgreementHistory, getEvidenceUrl, getMyAgreements, getMyOffers, getMyProfile, getMyRequests, getNotificationPreferences, getNotifications, getProjectMessages, getProjectUpdates, getProposalQuestions, getRequestMedia, getRequestOffers, getSession, handleCompletion, listPublicRequests, manageMilestone, manageRequestMedia, markNotificationsRead, performAgreementAction, proposeAmendment, requestLifecycleAction, respondAmendment, reviseOffer, saveNotificationPreferences, sendProjectMessage, setAgreementSchedule, signInWithEmail, signOut, submitOffer, submitReview, updateMyProfile, updateRequest, uploadRequestMedia, uploadWorkEvidence, withdrawOffer } from "./modules/backend.js";
import { discoverProfiles, getNetworkActivity, publishCompletion, setFollow } from "./modules/backend.js";
...
const store = createStore({ view: "discover", query: "", category: "All", selectedId: null, session: null, remote: false, profile: initial.profile, requests: initial.requests, myOffers: [], notifications: [], notificationPreferences: null, networkProfiles: [], networkActivity: [], networkQuery: "", networkExchange: "", networkRemote: false });
const { state } = store;
...
function agreementControls(agreement, confirmed) {
  ...
  const next = { agreed: ["scheduled", "Schedule work"], scheduled: ["active", "Start work"] }[agreement.status];
  const completion = agreement.status === "active" ? `<button class="secondary full" data-completion="request">Request completion approval</button>` : agreement.status === "review" && agreement.completion_requested_by !== state.profile.id ? `<button class="secondary full" data-completion="approve">Approve completion</button><button class="secondary full" data-completion="return">Return to active work</button>` : "";
...
function renderWorkspace() {
  const posted=state.requests.filter((r)=>r.ownerId===state.profile.id);const active=state.requests.filter((r)=>r.agreement&&!['completed','cancelled'].includes(r.status));const needsAction=active.filter((r)=>(r.status==='proposed'&&!r.agreement.confirmations.includes(state.profile.id))||(r.status==='review'&&r.agreement.completion_requested_by!==state.profile.id)||r.agreement.obligations?.some((o)=>o.status==='submitted'&&o.responsible_profile_id!==state.profile.id));const completed=state.requests.filter((r)=>r.status==='completed');
...
function renderNetwork() {
  const profiles=state.networkProfiles||[];const activity=state.networkActivity||[];
  return shell(`<section class="network-hero"><span class="eyebrow">Work network</span><h1>Find people through what they can do<br>and what they need next.</h1><p>Profiles are grounded in completed work, contextual reviews, and concrete offers—not follower counts.</p></section>
    ...
    <div class="two-col"><section><span class="eyebrow">Suggested collaborators</span><h2>${profiles.length} people with useful overlap</h2><div class="people-list">${profiles.map(networkPersonCard).join("")||`<div class="empty"><h3>No public profiles yet</h3><p>Try broader filters, or publish your own offers and needs.</p></div>`}</div></section><section><span class="eyebrow">Work activity</span><h2>Useful things moving forward</h2><div class="activity-list">${activity.map(activityCard).join("")||`<div class="empty"><p>Completed work and new public requests will appear here.</p></div>`}</div></section></div>`, "Community network");
}

function networkPersonCard(p){const caps=p.capabilities||[];const offers=caps.filter(x=>x.direction==='offer').map(x=>x.label);const needs=caps.filter(x=>x.direction==='need').map(x=>x.label);const myNeeds=(state.profile.needs||[]).map(x=>x.toLowerCase());const overlap=offers.filter(x=>myNeeds.some(n=>x.toLowerCase().includes(n)||n.includes(x.toLowerCase())));return `<article class="person-card"><span class="avatar big">${esc((p.display_name||'WT').split(/\s+/).map(x=>x[0]).join('').slice(0,2))}</span><div><h3>${esc(p.display_name)}</h3><small>${esc(p.location_text||'Location not listed')}${p.remote_available?' · Remote available':''}</small><p><b>Offers:</b> ${esc(offers.join(' · ')||'Not listed')}</p><p><b>Needs:</b> ${esc(needs.join(' · ')||'Not listed')}</p>${overlap.length?`<p class="match-reason">Matches what you need: ${esc(overlap.join(', '))}</p>`:''}<small>${p.completed_count||0} completed · ${p.review_count||0} reviews</small><div><button class="text-btn" data-view-profile="${p.id}">View evidence</button>${state.session&&p.id!==state.profile.id?`<button class="text-btn" data-follow-person="${p.id}">${state.profile.following?.includes(p.id)?'Following':'Follow'}</button>`:''}</div></div></article>`}
function activityCard(item){return `<article class="activity-card"><span class="category">${esc(item.type==='portfolio'?'Completed work':'Open request')}</span><h3>${esc(item.title)}</h3><p>${esc(item.summary||'')}</p><small>${esc(item.actor_name||'WorkTrade member')} · ${new Date(item.created_at).toLocaleDateString()}</small></article>`}
function publicProfileModal(p){const caps=p.capabilities||[];openModal(`<span class="eyebrow">Public work profile</span><h2>${esc(p.display_name)}</h2><p>${esc(p.bio||'No biography yet.')}</p><small>${esc(p.location_text||'Location not listed')}${p.remote_available?' · Remote available':''}</small><div class="two-col"><section><h3>Can offer</h3><div class="tags">${caps.filter(x=>x.direction==='offer').map(x=>`<span>${esc(x.label)}</span>`).join('')||'<p>None listed.</p>'}</div></section><section><h3>Needs</h3><div class="tags">${caps.filter(x=>x.direction==='need').map(x=>`<span>${esc(x.label)}</span>`).join('')||'<p>None listed.</p>'}</div></section></div><section><span class="eyebrow">Verified portfolio</span><div class="proof-grid">${(p.portfolio||[]).map(x=>`<div><b>${esc(x.title)}</b><p>${esc(x.summary)}</p><small>Linked to completed agreement</small></div>`).join('')||'<p>No public portfolio entries yet.</p>'}</div></section><section><span class="eyebrow">Contextual reviews</span>${(p.reviews||[]).map(x=>`<blockquote><p>${esc(x.body||'Completed successfully.')}</p><small>Reliability ${x.reliability}/5 · Communication ${x.communication}/5 · Quality ${x.work_quality}/5</small></blockquote>`).join('')||'<p>No published reviews yet.</p>'}</section>`)}
...
document.addEventListener("click", (event) => {
  ...
  if(action==="amend") amendmentModal(state.requests.find((x)=>x.id===state.selectedId));
  if(action==="add-milestone")milestoneModal(state.requests.find((x)=>x.id===state.selectedId));
  if(action==="schedule")scheduleModal(state.requests.find((x)=>x.id===state.selectedId));
  if(action==="compare-offers")compareOffersModal(state.requests.find((x)=>x.id===state.selectedId));
  if(action==="publish-completion")completionStoryModal(state.requests.find((x)=>x.id===state.selectedId));
  ...
  const completion=event.target.closest("[data-completion]");if(completion){const request=state.requests.find((x)=>x.id===state.selectedId);handleCompletion(request.agreement.id,request.agreement.version,completion.dataset.completion).then(loadRemoteWorkspace).then(()=>notify("Completion status updated")).catch((error)=>notify(error.message));}
  const amendment=event.target.closest("[data-amendment]");if(amendment){const [choice,id]=amendment.dataset.amendment.split(":");respondAmendment(id,choice==="accept").then(loadRemoteWorkspace).then(()=>notify(`Amendment ${choice}ed`)).catch((error)=>notify(error.message));}
  ...
  const followPerson = event.target.closest("[data-follow-person]"); if (followPerson) { const id=followPerson.dataset.followPerson;const following=state.profile.following||[];const shouldFollow=!following.includes(id);if(state.remote)setFollow(id,shouldFollow).then(()=>{state.profile={...state.profile,following:shouldFollow?[...following,id]:following.filter(x=>x!==id)};notify(shouldFollow?'Following collaborator':'Unfollowed collaborator')}).catch(error=>notify(error.message));else{state.profile={...state.profile,following:shouldFollow?[...following,id]:following.filter(x=>x!==id)};persist();notify("Following updated")} }
  ...
  const lifecycle=event.target.closest("[data-lifecycle]");if(lifecycle){event.stopPropagation();const[actionName,id,version]=lifecycle.dataset.lifecycle.split(":");requestLifecycleAction(id,Number(version),actionName).then(loadRemoteWorkspace).then(()=>notify(`Request ${actionName}d`)).catch((error)=>notify(error.message));}
  const editOffer=event.target.closest("[data-edit-offer]");if(editOffer){event.stopPropagation();reviseOfferModal(state.myOffers.find((o)=>o.id===editOffer.dataset.editOffer));}
  const withdraw=event.target.closest("[data-withdraw-offer]");if(withdraw){event.stopPropagation();if(confirm("Withdraw this proposal?"))withdrawOffer(withdraw.dataset.withdrawOffer).then(loadRemoteWorkspace).then(()=>notify("Proposal withdrawn")).catch((error)=>notify(error.message));}
  const removeMilestone=event.target.closest("[data-remove-milestone]");if(removeMilestone){event.stopPropagation();const request=state.requests.find((x)=>x.id===state.selectedId);manageMilestone(request.agreement.id,request.agreement.version,"remove",{milestone_id:removeMilestone.dataset.removeMilestone}).then(loadRemoteWorkspace).then(()=>notify("Milestone removed")).catch((error)=>notify(error.message));}
  const deleteMedia=event.target.closest("[data-delete-media]");if(deleteMedia){manageRequestMedia(deleteMedia.dataset.deleteMedia,"delete").then(loadRemoteWorkspace).then(()=>notify("Photo removed")).catch((error)=>notify(error.message));}
  const viewProfile=event.target.closest("[data-view-profile]");if(viewProfile){const profile=(state.networkProfiles||[]).find(x=>x.id===viewProfile.dataset.viewProfile);if(profile)publicProfileModal(profile)}
});
...
document.addEventListener("submit", async (event) => {
  ...
  if (form.dataset.form === "post") {
    ...
    if (state.remote) {
      try {
        const shouldPublish=event.submitter?.value!=="draft";const newId=await createRemoteRequest({ title: request.title, description: request.description, kind: request.category.toLowerCase(), location: request.location, urgency: request.urgency, cash_budget_cents: request.cashBudget * 100, visibility: "public",publish:shouldPublish, skills: request.skills,exchange_modes:exchanges,exchange_summary:request.offersInReturn.join(", "),constraints:data.get("constraints"),location_visibility:data.get("location_visibility") });const photo=form.elements.photo.files[0];if(photo){if(photo.size>10485760)throw new Error("Reference photo must be under 10 MB");await uploadRequestMedia(newId,photo,data.get("photo_caption"));}
        closeModal(); await loadRemoteWorkspace(); state.view = shouldPublish?"discover":"workspace"; notify(shouldPublish?"Work request published to the community":"Draft saved privately");
      } catch (error) { notify(error.message); }
  ...
  if (form.dataset.form === "offer") { if (state.remote) { try { const milestoneLines=String(data.get("milestones")||"").split("\n").map((x)=>x.trim()).filter(Boolean).map((title)=>({title,responsible:"provider",due_at:""}));await submitOffer(form.dataset.id, { mode: data.get("mode"), scope: data.get("gives"), exchange_summary: data.get("wants"), duration: data.get("duration"), exclusions:data.get("exclusions"),responsibilities:{provider:data.get("provider_supplies"),requester:data.get("requester_supplies")},milestones:milestoneLines,questions:data.get("questions"),expires_at:data.get("expires_at")?new Date(`${data.get("expires_at")}T23:59:59`).toISOString():"" }); closeModal(); await loadRemoteWorkspace(); notify("Detailed trade proposal sent"); } catch (error) { notify(error.message); } } else { updateRequests((list) => { const r = list.find((x) => x.id === form.dataset.id); r.offers.push({ id: crypto.randomUUID(), provider: state.profile.name, initials: state.profile.initials, mode: data.get("mode"), cash: Number(data.get("cash")) || 0, gives: data.get("gives"), wants: data.get("wants"), duration: data.get("duration"), note: data.get("questions") }); return list; }); closeModal(); notify("Trade proposal sent"); } }
  ...
  if (form.dataset.form === "update") { if(state.remote){try{await addProjectUpdate(state.selectedId,data.get("text"));form.reset();await loadRemoteWorkspace();notify("Project journal updated");}catch(error){notify(error.message)}}else{updateRequests((list) => { const r = list.find((x) => x.id === state.selectedId); r.updates.push({ id: crypto.randomUUID(), author: state.profile.name, text: data.get("text"), date: "Today" }); return list; }); form.reset(); notify("Update posted");} }
  ...
  if (form.dataset.form === "profile") { const profile = { ...structuredClone(state.profile), name: data.get("display_name"), location: data.get("location_text"), bio: data.get("bio"),workRadius:Number(data.get("work_radius_km"))||null,remoteAvailable:data.has("remote_available"),availability:data.get("availability_text"),resources:data.get("resources_text"),visibility:data.get("profile_visibility") }; try { if (state.remote) await updateMyProfile({ display_name: profile.name, location_text: profile.location, bio: profile.bio, needs: profile.needs, offers: profile.offers,work_radius_km:profile.workRadius,remote_available:profile.remoteAvailable,preferred_exchange_modes:["cash","barter","hybrid"],availability_text:profile.availability,resources_text:profile.resources,profile_visibility:profile.visibility }); state.profile = profile; if (!state.remote) persist(); closeModal(); notify("Profile saved"); } catch (error) { notify(error.message); } }
  ...
  if(form.dataset.form==="amendment"){try{await proposeAmendment(form.dataset.agreement,Number(form.dataset.version),{scope:data.get("scope"),exchange:{summary:data.get("exchange")},reason:data.get("reason")});closeModal();notify("Amendment sent for counterparty approval");}catch(error){notify(error.message)}}
  if(form.dataset.form==="milestone"){try{await manageMilestone(form.dataset.agreement,Number(form.dataset.version),"add",{title:data.get("title"),responsible_profile_id:data.get("responsible"),due_at:data.get("due_at")?new Date(`${data.get("due_at")}T12:00:00`).toISOString():""});closeModal();await loadRemoteWorkspace();notify("Milestone added");}catch(error){notify(error.message)}}
  if(form.dataset.form==="revise-offer"){try{const original=state.myOffers.find((o)=>o.id===form.dataset.id);await reviseOffer(form.dataset.id,{mode:data.get("mode"),scope:data.get("scope"),exchange_summary:data.get("exchange_summary"),duration:data.get("duration"),exclusions:data.get("exclusions"),responsibilities:original.responsibilities||{},milestones:original.proposed_milestones||[],questions:data.get("questions"),expires_at:data.get("expires_at")?new Date(`${data.get("expires_at")}T23:59:59`).toISOString():""});closeModal();await loadRemoteWorkspace();notify("Proposal revised");}catch(error){notify(error.message)}}
  if(form.dataset.form==="schedule"){try{await setAgreementSchedule(form.dataset.agreement,Number(form.dataset.version),{start_at:data.get("start_at")?new Date(data.get("start_at")).toISOString():"",timezone:data.get("timezone"),working_windows:data.get("working_windows")});closeModal();await loadRemoteWorkspace();notify("Schedule saved");}catch(error){notify(error.message)}}
  if(form.dataset.form==="proposal-question"){try{await askProposalQuestion(form.dataset.offer,data.get("body"));form.reset();notify("Question sent to proposal participants");}catch(error){notify(error.message)}}
  if(form.dataset.form==="network-search"){state.networkQuery=data.get('query')||'';state.networkExchange=data.get('exchange')||'';state.networkRemote=data.has('remote');await loadNetwork()}
  if(form.dataset.form==="completion-story"){try{await publishCompletion(form.dataset.agreement,data.get('summary'),data.get('exchange'),data.get('title'),data.get('visibility'));closeModal();await loadNetwork();notify("Verified completion story published")}catch(error){notify(error.message)}}
});
...
function mapRemoteRequest(request) {
  ...
  return { id: request.id, version: request.version, ownerId: request.owner_id, owner: name, initials: name.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase(), title: request.title, category: request.kind[0].toUpperCase() + request.kind.slice(1), location: request.location_visibility==="region"?(request.location_text || "Region not specified"):"Location shared with participants", distance: "—", urgency: request.urgency_text || "Flexible", status: request.stage, description: request.description,constraints:request.constraints||"", skills: (request.work_request_skills || []).map((item) => item.skill), exchange: request.exchange_modes||["cash","barter","hybrid"], cashBudget: request.cash_budget_cents ? Math.round(request.cash_budget_cents / 100) : 0, offersInReturn: request.exchange_summary?[request.exchange_summary]:["Open to a fair proposal"], createdAt: request.created_at, offers: [], updates: [], messages: [], followers: [], reports: [] };
}

async function loadRemoteWorkspace() {
  const [profile, requests,ownedRows,agreementRows,myOffers] = await Promise.all([getMyProfile(), listPublicRequests(),getMyRequests(), getMyAgreements(),getMyOffers()]);
  if (!profile) return;
  const capabilities = profile.capabilities || [];
  const combined=[...requests];for(const row of ownedRows)if(!combined.some((x)=>x.id===row.id))combined.push({...row,profiles:{display_name:profile.display_name}});const mapped=combined.map(mapRemoteRequest);await Promise.all(mapped.map(async(request)=>{request.media=await getRequestMedia(request.id)}));
  ...
  await Promise.all(ownedOpen.map(async (request) => { const offers = await getRequestOffers(request.id); request.offers = await Promise.all(offers.map(async(offer) => ({ id: offer.id, provider: offer.profiles?.display_name || "WorkTrade member", initials: (offer.profiles?.display_name || "WM").split(/\s+/).map((x) => x[0]).join("").slice(0,2), mode: offer.mode, gives: offer.scope, wants: offer.exchange_summary, duration: offer.duration_text || "To be agreed", note: offer.questions||"",exclusions:offer.exclusions,responsibilities:offer.responsibilities,expires_at:offer.expires_at,discussion:await getProposalQuestions(offer.id) }))); }));
  const participantRequests = mapped.filter((request) => request.agreement);
  await Promise.all(participantRequests.map(async (request) => { const [messages,updates]=await Promise.all([getProjectMessages(request.id),getProjectUpdates(request.id)]); request.messages = messages.map((message) => ({ id: message.id, authorId: message.author_id, author: message.profiles?.display_name || "Participant", text: message.body, date: new Date(message.created_at).toLocaleDateString() }));request.updates=updates.map((item)=>({id:item.id,author:item.profiles?.display_name||"Participant",text:item.body,date:new Date(item.created_at).toLocaleDateString()})); }));
  store.batch(() => {
    state.remote = true;
    state.profile = { id: profile.id, name: profile.display_name, initials: profile.display_name.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase(), location: profile.location_text || "", bio: profile.bio || "",workRadius:profile.work_radius_km,remoteAvailable:profile.remote_available,availability:profile.availability_text||"",resources:profile.resources_text||"",visibility:profile.profile_visibility, needs: capabilities.filter((item) => item.direction === "need").map((item) => item.label), offers: capabilities.filter((item) => item.direction === "offer").map((item) => item.label), following: [], joinedCircles: [], blocked: [] };
...
async function loadNetwork(){if(!backendConfigured)return;try{const [profiles,activity]=await Promise.all([discoverProfiles({query:state.networkQuery||'',exchange:state.networkExchange||null,remote:!!state.networkRemote}),getNetworkActivity()]);store.batch(()=>{state.networkProfiles=profiles||[];state.networkActivity=activity||[];if(state.session)state.profile={...state.profile,following:(profiles||[]).filter(x=>x.following).map(x=>x.id)}})}catch(error){notify(`Network unavailable: ${error.message}`)}}
