...
export async function getChangeOrderHub(id){const client=await getBackend();assertBackend(client);const{data,error}=await client.rpc("get_change_order_hub",{target_agreement_id:id});if(error)throw error;return data;}
export async function reportWorkIssue(id,payload){const client=await getBackend();assertBackend(client);const{data,error}=await client.rpc("report_work_issue",{target_agreement_id:id,payload});if(error)throw error;return data;}
export async function proposeChangeOrder(id,payload){const client=await getBackend();assertBackend(client);const{data,error}=await client.rpc("propose_change_order",{target_issue_id:id,payload});if(error)throw error;return data;}
export async function respondChangeOrder(id,accept){const client=await getBackend();assertBackend(client);const{error}=await client.rpc("respond_change_order",{target_change_order_id:id,accept});if(error)throw error;}
export async function manageWorkIssue(id,action){const client=await getBackend();assertBackend(client);const{error}=await client.rpc("manage_work_issue",{target_issue_id:id,issue_action:action});if(error)throw error;}
export async function uploadWorkIssueEvidence(agreementId,issueId,file,caption){const client=await getBackend();assertBackend(client);const session=await getSession();const ext=file.name.split(".").pop()?.toLowerCase()||"jpg";const path=`${agreementId}/${session.user.id}/issue-${crypto.randomUUID()}.${ext}`;const{error}=await client.storage.from("work-evidence").upload(path,file,{contentType:file.type,upsert:false});if(error)throw error;try{const{error:rpcError}=await client.rpc("add_work_issue_evidence",{target_issue_id:issueId,asset_path_value:path,caption_value:caption});if(rpcError)throw rpcError;return path;}catch(cause){await client.storage.from("work-evidence").remove([path]);throw cause;}}
