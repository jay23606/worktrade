...
export async function publishCompletion(agreementId,summary,exchangeBreakdown,title,visibility){const client=await getBackend();assertBackend(client);const{data,error}=await client.rpc("publish_completion",{target_agreement_id:agreementId,summary_text:summary,exchange_items:{summary:exchangeBreakdown||""},portfolio_title:title,portfolio_visibility:visibility});if(error)throw error;return data}
export async function discoverProfiles({query="",exchange=null,remote=false}={}){const client=await getBackend();assertBackend(client);const{data,error}=await client.rpc("discover_profiles",{search_text:query,exchange_filter:exchange,remote_only:remote});if(error)throw error;return data.map(row=>({...row.profile,capabilities:row.capabilities,portfolio:row.portfolio,reviews:row.reviews}))}
export async function getNetworkActivity(){const client=await getBackend();assertBackend(client);const{data,error}=await client.rpc("get_network_activity");if(error)throw error;return data.map(({activity:x})=>({type:x.kind,created_at:x.at,actor_name:x.name,...x}))}
export async function setFollow(profileId,follow){const client=await getBackend();assertBackend(client);const{error}=await client.rpc("set_follow",{target_profile_id:profileId,should_follow:follow});if(error)throw error}
