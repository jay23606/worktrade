...
- **P1 — Browser UAT absent:** 72 Playwright and axe checks now cover desktop, tablet, phone, zoom/reflow, target sizes, accessible names, modal keyboard behavior, safety reporting, navigation, activation and inbox routing, duplicate-submit protection, PWA behavior, themes, scroll preservation, and guided journeys in CI.
- **P1 — Project controls lacked a coherent sequence:** agreement, scheduling, preparation, active work, change-order, and completion controls now appear as one six-stage project path. It exposes one contextual next action, labels blocked and waiting states, and moves exceptional controls into a secondary tools disclosure.
- **P2 — Active projects became an undifferentiated long page:** project details now use responsive Overview, Activity, Exchange, and Files sections. Activity combines progress updates, participant messages, evidence events, and agreement decisions into one chronological stream while preserving separate update and message actions.
- **P2 — Notifications lacked prioritization and context:** the inbox now separates needs-action events, messages, and informational updates, labels whether the user is blocking progress, routes into the relevant project section, and allows routine project updates to be muted without hiding required actions.
- **P1 — New members faced the complete product at once:** the workspace now adapts to account maturity with a five-step first-exchange checklist, specific profile-readiness guidance, removable labeled examples, and a proposal form that teaches value, scope, responsibilities, exclusions, timing, and mutual confirmation. Completion is derived from real product records and therefore feeds the existing operator activation funnel without self-reported flags.
